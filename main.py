...
if __name__ == "__main__":
    ...
    if not os.path.exists("config.json"):
        ...
        print("config.jsonを作成しました。ボットトークンと許可ユーザーIDを設定してください。")
    
    # ボットの実行
    try:
        # 環境変数からトークンを取得（Secretsから）
        bot_token = os.getenv("DISCORD_BOT_TOKEN") or config.get("bot_token")
        if bot_token and bot_token != "YOUR_BOT_TOKEN_HERE":
            asyncio.run(setup_bot())
            bot.run(bot_token)
        else:
            print("DISCORD_BOT_TOKEN環境変数またはconfig.jsonにボットトークンを設定してください。")
    except Exception as e:
